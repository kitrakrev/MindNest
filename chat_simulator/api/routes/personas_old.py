...
async def list_personas(active_only: bool = True):
    """List all personas."""
    personas = persona_service.list_personas(active_only)
    return personas
...
async def deactivate_persona(persona_id: str):
    ...
    success = persona_service.deactivate_persona(persona_id)
    if not success:
        raise HTTPException(status_code=404, detail="Persona not found")
    return {"message": "Persona deactivated successfully"}
...
async def add_memory(
    ...
    success = persona_service.add_memory(persona_id, content, memory_type, importance)
    if not success:
        raise HTTPException(status_code=404, detail="Persona not found")
    return {"message": "Memory added successfully"}


@router.post("/import", response_model=List[Persona])
async def import_personas(file: UploadFile = File(...)):
    ...
    try:
        ...
        if not isinstance(personas_data, list):
            raise HTTPException(
                status_code=400,
                detail="File must contain a JSON array of personas"
            )
            
        personas_create = [PersonaCreate(**data) for data in personas_data]
        imported = persona_service.import_personas(personas_create)
        return imported
    ...
    try:
        ...
        if request.auto_create:
            for persona_data in personas_data:
                ...
                if persona_service.persona_name_exists(persona_data.name):
                    skipped.append(persona_data.name)
                    # Add existing persona to preview
                    existing = persona_service.get_persona_by_name(persona_data.name)
                    if existing:
                        preview.append({
                            "id": existing.id,
                            "name": existing.name,
...
async def generate_from_description(request: PersonaGenerationRequest):
    ...
    try:
        ...
        if persona_service.persona_name_exists(request.name):
            raise HTTPException(
                status_code=400, 
